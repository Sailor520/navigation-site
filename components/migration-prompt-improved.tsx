...
import { useDataMigrationImproved } from '@/lib/data-migration-improved'

interface MigrationPromptProps {
  onClose?: () => void
}

export function MigrationPromptImproved({ onClose }: MigrationPromptProps) {
  const { 
    shouldShow, 
    hasSupabaseConfig, 
    isLoading, 
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-lg">Supabase 配置缺失</CardTitle>
            </div>
            {onClose && (
  ...
  const handleClearLocalData = () => {
    if (confirm('确定要清理本地数据吗？这个操作不可撤销。')) {
      clearLocalData()
            ...
            {showDetails && (
              <Alert>
                ...
                <AlertDescription className="text-xs">
                  ...
                  <br />4. 标记迁移完成
                  <br /><br />
                  <strong>注意：</strong>迁移过程是安全的，我们会先备份您的数据。迁移完成后，本地数据将保留作为备份。
                </AlertDescription>
              </Alert>
            )}
          </div>
